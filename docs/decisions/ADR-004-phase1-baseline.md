...
### 4.2 实测:改写 eval 对比泄漏程度

为量化泄漏到底有多严重,我们用 DeepSeek 把 eval_v1 的每条 question 改写成口语化、
避开原问题关键词的版本(gold_doc_ids 不变),得到 `eval_v1_paraphrased.jsonl`。

改写示例:
- 原: "阿里巴巴集团中哪个业务是专注于限时促销的销售和营销平台?"
  改: "阿里旗下那个专门搞限时抢购的平台叫啥来着?"
- 原: "亚索在当前版本为什么是一个非 ban 必选的存在?"
  改: "这版本为啥大家都要抢着选那个拿剑的浪人?"

**对比结果**:

| Metric | eval_v1 (泄漏版) | eval_v1_paraphrased (抗泄漏) | Δ |
|--------|-----------------|-----------------------------|-----|
| Recall@3 | 0.9310 | 0.8740 | -0.0570 |
| Recall@5 | 0.9572 | 0.9152 | -0.0420 |
| Recall@10 | 0.9755 | 0.9388 | -0.0367 |
| Hit@5 | 1.0000 | 0.9600 | -0.0400 |
| MRR | 0.9758 | 0.9116 | -0.0642 |

**结论**:
1. 泄漏存在,**词面匹配让 Recall@3 虚高 ~6 个百分点**
2. 但 BGE-M3 的语义匹配能力是真的强,**改写后 87% 仍然很高**——
   说明这套 baseline 在该语料上确实接近上限,纯 dense 已经把大部分语义都吃到了
3. Phase 2 的混合检索 / 多粒度提升空间存在,但**别期待 +8pp 那么大**;
   实际更可能是 +1~3pp(用 paraphrased 集做对比更可信)
4. **简历叙事**应当报告 paraphrased 版本(87%)而非 eval_v1(93%),诚实更有说服力
